...
def get_center_and_extent(my_halo, 
    ...
    if "hdf5" not in output_format and \
       "txt" not in output_format and \
       None not in output_format:
    ...
    for grid in pf.index.grids[my_work]:
        ...
        my_indices = np.isin(grid_p_indices, halo_indices)
        ...
        for i, axis in enumerate(axes):
            ...
            halo_indices = np.setdiff1d(halo_indices,
                                        halo_indices[np.isin(halo_indices,
                                                             grid_p_indices)])
    ...
    if my_rank == 0:
        ...
        if "hdf5" in output_format:
            output_fn += ".h5"
            fp = h5.File(output_fn, "a")
            fp["particle_position"] = all_save_pos.T
            fp["particle_identifier"] = all_indices.astype('int').v
            fp.close()
            written = True
        if "txt" in output_format:
            output_id_fn = output_fn + "-id.dat"
            output_fn += ".dat"
            np.savetxt(output_fn, all_save_pos.T)
            np.savetxt(output_id_fn, all_indices)
            written = True
        if output_format[0] == None:
            output_fn = None
        elif not written:
...
if __name__ == '__main__':
    my_halo = {'id': 26813,
               'center': ([0.5117795, 0.5010789, 0.490334], 'Mpccm'),
               'mass': (1347916.0, 'Msun'),
               'r_units': 'kpc'}

    get_center_and_extent(my_halo, 'DD0000/output_0000', 'DD0125/output_0125',
                          round_size=64, radius_factor=3.0, output_format=['hdf5', 'txt'])
